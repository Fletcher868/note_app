import PocketBase from 'pocketbase'; // PocketBase uses Default export (OK)
import { Editor } from '@tiptap/core'; // Named export (OK)
import StarterKit from '@tiptap/starter-kit'; // StarterKit uses Default (OK)

// CHANGE THESE TO USE CURLY BRACES { }
...
menu.querySelector('.ctx-share').onclick = async () => {
  console.error('[EXECUTING]', new Error().stack.split('\n')[1].trim().split(' ')[1]);
  menu.remove();
  
  // Select the file and wait for the decryption process to complete
  await selectFile(id);
  
  // Now that the content is decrypted and available in state, open the modal
  openShareModal(id);
};

// 2. Update Download handler
menu.querySelector('.ctx-download').onclick = async () => {
  console.error('[EXECUTING]', new Error().stack.split('\n')[1].trim().split(' ')[1]);
  menu.remove();
  
  // Select the file and wait for decryption
  await selectFile(id);
  
  // Get the updated file object from state (which now contains the decrypted content)
  const freshFile = state.files.find(f => f.id === id);
  if (freshFile && freshFile.content !== null) {
    downloadNote(freshFile);
  }
};
